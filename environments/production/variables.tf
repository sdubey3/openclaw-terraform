...
variable "public_key" {
  description = "SSH public key content for EC2 access. Leave empty to disable SSH key pair creation."
  type        = string
  default     = ""
}

variable "ssh_allowed_cidr" {
  description = "CIDR block allowed to SSH on port 22. Leave empty for no SSH access (SSM only). Use /32 for single IP (e.g., 203.0.113.1/32)."
  type        = string
  default     = ""

  validation {
    condition     = var.ssh_allowed_cidr == "" || can(cidrhost(var.ssh_allowed_cidr, 0))
    error_message = "SSH allowed CIDR must be empty or a valid CIDR notation (e.g., 203.0.113.1/32 for single IP)."
  }
}
